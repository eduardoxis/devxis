...
import {collection,getDocs,query,where,orderBy,limit,startAfter} from 'https://www.gstatic.com/firebasejs/10.14.1/firebase-firestore.js';
...
function readCache(key){try{const data=JSON.parse(sessionStorage.getItem(CACHE_PREFIX+key));return data&&Date.now()-data.at<CACHE_MS?data.value:null}catch{return null}}
function saveCache(key,value){try{sessionStorage.setItem(CACHE_PREFIX+key,JSON.stringify({at:Date.now(),value}))}catch{}}
function mapDocs(snapshot){return snapshot.docs.map(doc=>({id:doc.id,...doc.data()}))}
...
export async function getProjectsPage(cursor=null,pageSize=6){
  ...
  const docs=snapshot.docs;const hasMore=docs.length>pageSize;const visible=docs.slice(0,pageSize);
...
export async function getPublished(name,fallback=[]){
  ...
  const items=mapDocs(snapshot);saveCache(name,items);return items;
}
